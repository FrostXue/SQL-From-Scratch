...
namespace ECE141 {
  ...
  struct Token {
    ...
    Token(TokenType aType = TokenType::unknown, Keywords aKw = Keywords::unknown_kw, 
        Operators anOp = Operators::unknown_op, std::string aData = "") :
        type(aType),keyword(aKw),op(anOp),data(aData){}
    Token(const Token& aCopy) {
        operator=(aCopy);
    }
    TokenType   type;
    Keywords    keyword;
