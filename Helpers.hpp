...
namespace ECE141 {
  ...
  template<typename T, size_t aSize> // the function can automatically determine the size of the array you pass to the function!
  ...
  public:
    ...
    static std::string toString(Entry anEntry){
        const int kInt = 0;
        const int kDouble = 1;
        const int kChar = 2;
        const int kString = 3;
        ...
        switch (anEntry.index())
        {
        case kInt:
			if (std::get<int>(anEntry) == -1) return "NULL";
            else return std::to_string(std::get<int>(anEntry));
        case kDouble:
            return std::to_string(std::get<double>(anEntry));
        case kChar:
            return std::to_string(std::get<char>(anEntry));
        case kString:
            return std::get<std::string>(anEntry);
        case kBool:
            return std::get<bool>(anEntry) ? "TRUE" : "FALSE";
        default:
            return "NULL";
            break;
        }
    ...
    static std::vector<std::string> toString(Variants aRow){
        std::vector<std::string> toReturn;
        for (Entry theEntry: aRow){
            toReturn.push_back(toString(theEntry));
        }
        return toReturn;
    }

	static Entry stringToVariant(std::string aValue, int anIndex) {
		const int kInt = 0;
		const int kDouble = 1;
		const int kChar = 2;
		const int kString = 3;
		const int kBool = 4;
		//std::clog << "stringToVariant(aValue: " << aValue << ", anIndex: " << anIndex << "\n";
		switch (anIndex) {
		case kInt:
			if (aValue == "NULL") return Entry(-1);
			return Entry (std::stoi(aValue));
		case kDouble:
			return Entry(std::stod(aValue));
		case kChar:
			return Entry(aValue[0]);
		case kString:
			return Entry(aValue);
		case kBool:
			return Entry((aValue[0] == 't' || aValue[0] == 'T') ? true:false);
		default:
			return Entry("Null");
			break;
		}
    ...
    static std::string toLower(const std::string& aStr) {
        std::string theCopy = aStr;
        std::transform(theCopy.begin(), theCopy.end(), theCopy.begin(),
            [](unsigned char c) { return std::tolower(c); });
        return theCopy;
    }

   /* template <typename T>
    static std::vector<T> Erase(const std::vector<T>& aVec, uint32_t aPos, uint32_t aLength) {
        std::vector<T> toReturn;
        for (uint32_t i = aPos; i < aPos + aLength; i++) {
            toReturn.push_back
        }
    }*/
  };
