...
namespace ECE141 {
  ...
  StatusResult Tokenizer::tokenize() {
    ...
    while(!input.eof()) { // while the stream is not at the end yet
       
      theChar = input.peek(); // peek next letter
      if (isPunctuation(theChar)) { // the char is punctuation
          ...
          std::string temp; // never initialized
          temp += theChar;
          Token theToken{ TokenType::operators };
          theToken.data.push_back(input.get());
          theToken.op = Helpers::toOperator(temp);
          tokens.push_back(theToken);
      }
      else if (isNumber(theChar)) { // parse number in string
          Token theToken{ TokenType::number, Keywords::unknown_kw, Operators::unknown_op };
          theToken.data = readWhile(isNumber);
          tokens.push_back(theToken);
      }
      else if (isQuote(theChar) || (apostrophe == theChar)) { // parse string in quotes
